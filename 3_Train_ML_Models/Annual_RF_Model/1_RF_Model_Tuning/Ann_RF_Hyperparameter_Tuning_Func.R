...
# This script creates a function to tune random forest for the annual model, optimizing RF hyperparameters on 80% training set using stratified sampling by groups of basins
...
rf_func_hyperparameter_tuning <- function(metric){
  ...
  strat_basin_train = strat_basin_ann[which(folds != 1),]
  ...
  mtry_oob <- data.frame(mtry_val = mtry_val, cv1 = rep(0,20), cv2 = rep(0,20), cv3 = rep(0,20), cv4 = rep(0,20), cv5 = rep(20))
