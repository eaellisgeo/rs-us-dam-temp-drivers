...
# Create function to run random forest on given metric and season
# 80/20 Training Set using Geographic Stratified sampling
...
rf_func <- function(metric, season){
  ...
  # If metric is for Directly Upstream
  if (metric == "Change"){
    ...
    seas_dams_filepath <- paste('F:/Insert_File_Path/RF_Model_Results/Dams_Used/Dams_',i,'.csv',sep="") ## Update file path here
    write.csv(strat_basin_seas,seas_dams_filepath)
